...
Sub Save()
    ...
    savePath = Sheets("Code").Range("E1").Value & "\" & ActiveSheet.Name & " " & timeStamp & ".xlsx"
...
'Insert MD column
Sheets("PAX").Range("I:I").EntireColumn.Insert
...
For i = 2 To 1000
    If Sheets("Sys1").Range("R" & i).Value <> "HK" And Sheets("Sys1").Range("r" & i).Value <> "" Then
        With Sheets("Sys1").Range("R" & i)
            .Font.Color = RGB(255, 0, 0)
            .Font.Bold = True
...
''INTERLINE
For i = fr To lr
    If Sheets("PAX").Range("B" & i).Text Like "###-########" And Not Sheets("PAX").Range("B" & i).Text Like "157-########" Then
...
'Delete MD
Sheets("PAX").Range("I:I").EntireColumn.Delete
...
''INTERLINE
For i = fr To lr
    If Sheets("FRT").Range("B" & i).Text Like "###-########" And Not Sheets("FRT").Range("B" & i).Text Like "157-########" Then
...
'Clear not DocNo
Dim awbcell As Range
Dim mlr As Integer

mlr = Sheets("AWB").Range("M1000").End(xlUp).Row
For Each awbcell In Sheets("AWB").Range("M3:M" & mlr)
    If Not awbcell.Text Like "###-########" Then
        awbcell.Clear
    End If
Next awbcell

'Clear OffBal & Cut
Dim ob, ct As Boolean
For i = 3 To mlr
    ob = Application.WorksheetFunction.CountIfs(Sheets("Offbal_Partial").Range("A2:A1000"), Sheets("AWB").Range("M" & i)) > 0
    ct = Application.WorksheetFunction.CountIfs(Sheets("Offbal_Partial").Range("C2:C1000"), Sheets("AWB").Range("M" & i)) > 0
    If ob Or ct Then
        Sheets("AWB").Range("M" & i).Clear
    End If
Next i

'No missing found
If Application.WorksheetFunction.CountA(Sheets("AWB").Range("M3:M" & mlr)) = 0 Then
    ...
    End With
Else
    Sheets("AWB").Range("M3:M" & mlr).RemoveDuplicates Columns:=1
    Sheets("AWB").Range("M3").Delete Shift:=xlUp
End If
...
'Change format
For i = fr To lr
    Sheets("NOTOC").Range("B" & i).Value = Format(Sheets("NOTOC").Range("B" & i).Value, "###-########")
Next i
